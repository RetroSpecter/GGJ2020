...
float edgeCalc(float2 UV, float OutlineThickness, float opacity) {
	float halfScaleFloor = floor(OutlineThickness * 0.5);
	float halfScaleCeil = ceil(OutlineThickness * 0.5);
	float2 Texel = (1.0) / float2(_CameraColorTexture_TexelSize.z, _CameraColorTexture_TexelSize.w);
	...
	float depthFiniteDifference1 = depthSamples[3] - depthSamples[2];
	float edgeDepth = sqrt(pow(depthFiniteDifference0, 2) + pow(depthFiniteDifference1, 2)) * 100;
	float depthThreshold = (1 / -1) * depthSamples[0];
	edgeDepth = edgeDepth > depthThreshold ? opacity : 0;

	return edgeDepth;
}

void Cutout_float(float2 UV, float OutlineThickness, float4 OutlineColor, out float4 Out)
{
	float edge = 0;

	
	for (int i = 0; i <= OutlineThickness; i += OutlineThickness / 2) {
		edge += edgeCalc(UV, i, OutlineColor.a / 2);
		edge = clamp(edge, 0, 1);
	}
	
	

	edge -= edgeCalc(UV, 0, 1);
	//edge = edgeCalc(UV, OutlineThickness, 0.5f) - edgeCalc(UV, 0, 1);
	edge = clamp(edge, 0, 1);
	...
	float4 original = SAMPLE_TEXTURE2D(_CameraColorTexture, sampler_CameraColorTexture, uvSamples[0]);
	//Out = ((1 - edge) * original) + (edge * lerp(original, OutlineColor, OutlineColor.a));
	Out = (edge * lerp(float4(0,0,0,0), OutlineColor, OutlineColor.a));
}
